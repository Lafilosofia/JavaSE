...
public class Clinet3 {
	...
	private void start(){
		...
		try {
			 ...
			 * 
			 */
			ServerHandler3 server = new ServerHandler3(socket);
			//该线程负责接收服务端发送过来的消息
...
//该任务负责接收服务端发送过来的消息
class ServerHandler3 implements Runnable {
	private Socket socket;
	
	public ServerHandler3(Socket socket){
		this.socket = socket;
	}
